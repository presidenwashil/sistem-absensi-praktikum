		...
		<div class="container-login100">
			<div class="wrap-login100">
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					if ($level == 1) {
						// Aslab
						$sqlCek = mysqli_query($con, "SELECT * FROM tb_aslab WHERE nia='$_POST[username]' AND password='$pass' AND status='Y'");
						$jml = mysqli_num_rows($sqlCek);
						$d = mysqli_fetch_array($sqlCek);

						if ($jml > 0) {
							$_SESSION['aslab'] = $d['id_aslab'];
							echo "
						<script type='text/javascript'>
						setTimeout(function () { 
						
						swal('($d[nama_aslab]) ', 'Login berhasil', {
						icon : 'success',
						buttons: {        			
						confirm: {
						...
						// staff
						$sqlCek = mysqli_query($con, "SELECT * FROM tb_staff WHERE nip='$_POST[username]' AND password='$pass' AND status='Y'");
						$jml = mysqli_num_rows($sqlCek);
						$d = mysqli_fetch_array($sqlCek);
